...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/21 20:22:42 by akekesi           #+#    #+#             */
/*   Updated: 2023/09/21 20:27:21 by akekesi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	so_long_demo(void)
{
    ...
    // open map_track.ber
    fd = open("map_track.ber", O_RDONLY);
    if (fd < 0)
        return ;
    ...
    while (i--)
    {
        ...
        node = llist_create(ft_strdup(map_track->value));
        llist_add(&map, node);
        llist_rrot(&map);
        llist_print(map);
        ft_putstr("\n");
