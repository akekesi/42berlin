...
/*   Updated: 2023/01/19 15:08:00 by akekesi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1)
		return (ft_str_free(&str_line[fd]));
	...
	str_buff[n] = '\0';
	if (n == 0)
		return (ft_get_next_line_sub1(&str_buff, &(str_line[fd])));
	return (ft_get_next_line_sub2(&str_buff, &(str_line[fd]), fd));
}
